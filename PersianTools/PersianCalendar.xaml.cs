...
using PersianTools.JalaliTools;
...
namespace PersianTools
{
    ...
    public partial class PersianCalendar : UserControl
    {
        ...
        static PersianCalendar()
        {
            ...
            {
                DefaultValue = PersianDate.MinValue,
            ...
            {
                DefaultValue = PersianDate.MaxValue,
        ...
        private void setMonthMode()
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                for (int j = 0; j < 7; j++)
                {
                    ...
                    button.IsEnabled = dateInRange;
                    button.Content = dateInRange ? date.Day.ToString() : "";
                    button.Tag = dateInRange ? date : default(PersianDate?);
                    if (date == PersianDate.Today)
                    {
                        button.Foreground = Brushes.BlueViolet;
                    }
                    else if (dateInRange && date.Month == firstDayInMonth.Month)
                    {//we're good!
